...
def pairwise_squared_distance(x: th.Tensor, y: th.Tensor) -> th.Tensor:
    '''
    pairwise squared (L2) distances between two sets of vectors
    Args:
        x: Tensor(N, C)
        y: Tensor(M, C)
    '''
    yt = y.t()
    dist = x.pow(2).sum(1, keepdim=True) + yt.pow(2).sum(0, keepdim=True)
    dist.addmm_(1, -2, x, yt)
    return dist
